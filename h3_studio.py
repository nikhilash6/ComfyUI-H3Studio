...
class H3StudioImageToVideo(io.ComfyNode):
    ...
    def define_schema(cls):
        return io.Schema(
            ...
            inputs=[
                ...
                io.Boolean.Input("motion_context_anchor_brightness", default=True,
                    tooltip="ON by default. Stops a chain's brightness drifting, by correcting the CONDITIONING instead of the finished pixels.\n\nThe pinned context is what the model matches, so before pinning it the node measures what the previous render actually came out at and nudges the context by the error against the chain's anchor, taken from its first link. Nothing assumes how much a link drifts: 0.5% drift gets a 0.5% correction, 4% gets 4%, and if the model only partly follows the nudge the residue is corrected next link -- it converges instead of accumulating.\n\nMeasured in PIXELS off the previous render, so it works whether or not the latent cache hit -- and per region, since a sky that drifts while the road doesn't is a case one number cannot express.\n\nCosts nothing (an add on a tensor already in memory), never touches finished pixels (so no clipped highlights and no clip paying for the ones before it), and it fixes the render FILES, not just the export. Clamped to +-0.08 so a deliberate lighting change isn't fought; clear the reel to re-anchor.\n\nTurn it off to render exactly what the model produces."),
    ...
    def execute(cls, clip, vae, prompt, width, height, length,
                ...
                motion_context_anchor_brightness=True, motion_context_strength=0.92,
