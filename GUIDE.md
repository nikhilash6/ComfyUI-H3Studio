...
No — no time and no repeated frame. The frames just after a pinned head come out unstable (the first free latent step is decoded using the pinned step as its context, so it wears the *previous* clip's exposure — that's what a flash at a cut is). Instead of dropping them and losing that fraction of a second, the node stops **pinning** a couple of frames before the previous clip stops **playing**, so those instants are shown from the previous clip's own real frames. Automatic; the log says `safe tail bridge` when it applies.
