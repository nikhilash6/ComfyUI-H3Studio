...
Plain **⏭** hands the next clip a still frame — the model re-decides speed and direction from scratch, and audio is only ever *imitated*. **⏭▶ with motion** (on reel cards and the render dock) instead pins the previous clip's **tail frames + audio** at the new clip's head on its own timeline: same motion, same direction, the **same waveform continued**. Costs extra conditioning rows (the **⏭▶ MOTION bar** has the dials — 22 frames is the sweet spot, 5 is cheap) and the render opens by repeating the pinned tail, which **🎞 add to reel auto-trims** so an export never duplicates the join. The pinned span shows hatched at the head of the timeline. Sound dulls slightly at every motion-join down a long chain (each clip regenerates the last one's output) — restart the chain at a natural transition when you hear it. Technique credit: the **ComfyUI-H3-Motion-Context** pack, whose seam-probe work proved the audio-timeline trick.
...
Hit **▶ queue**: a progress strip appears in the header, the sampling preview streams into a corner dock, and when it finishes the clip **plays right there** — with **⏭ last frame → first**, **⏭▶ with motion** and **+ as video ref** buttons to chain straight into the next shot.

### I want to chain clips into one long video (and cut it)
Every **⏭ continue** docks the source clip into the **REEL** strip at the bottom; add renders from the result dock's **🎞 add to reel**. Each card has an **iOS-style trim strip** — drag the green handles to set in/out (the preview loops the kept range), the **⧉ dial between cards** sets a crossfade, and **fade in / out** on the header bookend the whole piece. Nothing is baked: every trim, crossfade and fade stays editable and only applies at **⇧ export as one video** — a single mp4 (equal-power audio blends, de-clicked cuts) in `output/h3reel/`, no external tools. **⏭** continues from a clip's OUT point (**⏭▶** does it with motion + audio continuity), **🎲 re-roll last** replaces the newest take.
