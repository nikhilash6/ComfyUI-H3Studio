...
### I want motion (and the actual sound) to continue across a join
Plain **⏭** hands the next clip a still frame — the model re-decides speed and direction from scratch, and audio is only ever *imitated*. Turn on **⏭▶ continue the reel** (the MOTION bar): from then on, **every ▶ queue continues from the reel's newest clip**, resolved at queue time — its tail frames + audio are pinned at the new clip's head on its own timeline, so same motion, same direction, the **same waveform continued**. The loop is just *render → 🎞 add to reel → queue again*; a trimmed card continues from its OUT point, and 🎲 re-roll naturally continues from the clip before the one it replaces. **pick clip…** continues from a specific file instead (turns the toggle off). Costs extra conditioning rows (dials on the bar — 22 frames is the sweet spot, 5 is cheap); the render opens by repeating the pinned tail, which **🎞 add to reel auto-trims** so an export never duplicates the join. The pinned span shows hatched at the head of the timeline. Sound dulls slightly at every motion-join down a long chain (each clip regenerates the last one's output) — restart the chain at a natural transition when you hear it. Technique credit: the **ComfyUI-H3-Motion-Context** pack, whose seam-probe work proved the audio-timeline trick.
...
Every **⏭ continue** docks the source clip into the **REEL** strip at the bottom; add renders from the result dock's **🎞 add to reel**. Click a card's **✂ readout** to trim it in a **big popup view** — full-size preview, draggable in/out handles, the kept range loops as it plays. The **⧉ dial between cards** sets a crossfade, and **fade in / out** on the header bookend the whole piece. Nothing is baked: every trim, crossfade and fade stays editable and only applies at **⇧ export as one video** — a single mp4 (equal-power audio blends, de-clicked cuts) in `output/h3reel/`, no external tools. **⏭** continues from a clip's OUT point (turn on **⏭▶ continue the reel** for motion + audio continuity at every queue), **🎲 re-roll last** replaces the newest take.
