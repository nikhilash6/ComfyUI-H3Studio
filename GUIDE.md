...
**♪ music** in the TIMELINE header: pick an audio file (upload, input folder, mic, or the free web search) and its **waveform + detected hits** draw on the lane. (Careful with words here: **"text beats"** are your timed prompt lines; **"hits"** are the green ticks the music detection finds. They share a lane, not a meaning.) **▶** auditions exactly the window under this clip, and the **use** dropdown says what the music is for — *timing only* (default), *soundtrack* (the song drops onto the ♪ lane in AUDIO, one chip per clip), *model reference* (the model imitates its character), or both — place waypoints and beats on the music by eye, or turn on **snap ♪** and they click onto the nearest hit as you drag. **at N s / follow reel** controls which slice of the song sits under this clip — *follow reel* tracks the reel's running duration, so each new clip automatically sees its own part of the track. On *timing only* the music changes nothing about the render or the export — it's there to aim at. Each reel card has its **own volume** (🔊 slider under the thumbnail) for the clip's *generated* soundtrack — drop it, or hit 🔇 to mute that clip entirely, when the soundtrack and fx lanes are carrying the piece. **🔇 mute clips** in the header does all of them at once (click again to restore). Export-only; the render files are untouched, and ▶ play reel previews the levels you set. To put the song IN the finished piece, set **use** to *soundtrack*: the song lands on the **♪ lane** in AUDIO, and each clip carries its own slice of it — **cut to that clip's length**, so a five minute track behind a four second clip is four seconds of chip, not five minutes. Clips already in the reel are filled in immediately, playing straight on from one another; every clip you add afterwards brings its own slice with it. From there it's an edit: drag a chip along the lane, drag either end to trim, click it for volume and fades. **▶ play reel** previews the whole mix live.
...
### Is anything lost at a motion join?
No time is lost. The handover is snapped to a latent-step boundary (a pinned run can only end on one), so the next clip pins everything the previous one shows — otherwise a car entering frame in the last few frames would be invisible to the model and vanish at the cut. The first delivered frame carries the pinned head's exposure rather than the one the model settles on, and that is corrected by level at export: **✨ luma-match join**, on by default for continued clips, over a short window so it can't drag the frames after it. Untick it in the ✂ popup to see the raw render.

### My chain keeps going static
H3 often stops moving before a clip ends, and continuing from that frozen tail makes the next clip open on a still — which then gets pinned by the link after it. Clips are now scanned as they join the reel, and if one locks up the card's **out-trim** is set to where the motion stops: the export drops the freeze and **⏭▶** continues from live motion, because both read that same out-trim. It never overrides a trim you set, and you can drag it back if you disagree. Watch the log for `the N frames being pinned are frozen` if you drive the node without the editor. Detection compares each frame against a median of the clip's final frames rather than looking for low motion — H3 decelerates into its ending, so a motion threshold cuts far too early.
...
Set the **section** seconds in the editor's V2V row — only that stretch is restyled and the render IS that section. Pin its first/last frames from the untouched footage so the joins can't drift, then rejoin it to the original in your editor of choice.
...
**AUDIO** under the reel: four overlay lanes spanning the whole reel, each holding **as many or as few files as you like** — one long bed stretched across the piece, or a dozen stabs. **♪** is the soundtrack lane, where a clip's own music lands when you add the clip (see above); **fx1–fx3** are yours. **+** on a lane opens the audio picker (files, upload, mic, free web search); the sample lands as a **chip you drag into place** — band hit at 4s on fx1, car engine at 12s on fx2, they overlay freely. Editing is by hand, not by typing: **drag the middle** of a chip to move it, **drag either end** to trim it (dragging the head keeps the sound where it sits on the reel and eats into the file instead). Click a chip for **vol %** (per sample, up to 150) and **fade in/out** (its own ease), with the position and slice shown beside them; **▶** auditions it, **✕** removes it. Everything is mixed at **⇧ export** on top of clip audio + music bed, and **▶ play reel** previews the lot live. The arrangement saves with the workflow and with **💾 save setup** files (a reel card's ⚙ deliberately leaves it alone).
